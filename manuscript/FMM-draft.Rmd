...
`r nrow(filter(df, post_PWM >= lambda_max(df$post_PWM, 0.05)))` participants had posterior probabilities of assignment to PWM that were greater than the established threshold for statistically significant classification to that theory, where `r nrow(filter(df, post_PMT >= lambda_max(df$post_PMT, 0.05)))` participants exceeded that threshold for PMT. This indicates that overall `r nrow(filter(df, post_PWM >= lambda_max(df$post_PWM, 0.05))) + nrow(filter(df, post_PMT >= lambda_max(df$post_PMT, 0.05))) / nrow(df) * 100`% of participants held intentions that were best explained by a single theory (in this case PWM), and that instead the majority of participants would be better explained by a combination of the two theories - each to a greater or lesser extent.
