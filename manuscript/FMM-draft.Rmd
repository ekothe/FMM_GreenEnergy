...
### Pre-registered analyses
Figure \@ref(fig:probdistbeh) illustrates the distribution of individuals’ posterior probabilities for being consistent with each theory. Examining the mean posterior probability of classification using each theory indicates that on average, individuals’ intention to use green energy is predominately influenced by PWM processes ($\mu$ PWM = `r mean(df$beh_post_PWM)`; median PWM = `r median(df$beh_post_PWM)`); however, on the whole individuals are still influenced by PMT variables to a substantial degree ($\mu$ PMT = `r mean(df$beh_post_PMT)`; `r median(df$beh_post_PMT)`). Overall, `r nrow(filter(df, beh_cluster == "PWM"))/nrow(df) * 100`% of individuals have posterior probabilities that are more consistent with PMT than PWM (i.e. $\zeta$~PMT~~,~ ~i~ > .5; $\zeta$~PWM~~,~ ~i~ < .5) and are therefore classified as consistent with PMT based on maximum posterior probability group assignment. However, given that the posterior probabilities are clustered near 0.5 no participants were classified as statistically significantly classified as consistent with either theory.
